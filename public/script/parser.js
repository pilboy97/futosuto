...
function isSceneBeginOpen() {
  if (header + 1 >= code.length) return false

  if (code[header].kind !== Kind.LE) return false
  if (code[header + 1].kind !== Kind.LE) return false
...
function isSceneBeginClose() {
  ...
  if (code[header].kind !== Kind.GR) return false
  if (code[header + 1].kind !== Kind.GR) return false

  header += 2
  return true
}
function isSceneEnd() {
  if (header + 4 >= code.length) return false

  if (code[header].kind !== Kind.LE) return false
  if (code[header + 1].kind !== Kind.LE) return false
  if (code[header + 2].kind !== Kind.DIV) return false
  if (code[header + 3].kind !== Kind.GR) return false
  if (code[header + 4].kind !== Kind.GR) return false
...
function isOptionEnd() {
  if (header + 2 >= code.length) return false

  if (code[header].kind !== Kind.LE) return false
  if (code[header + 1].kind !== Kind.DIV) return false
  if (code[header + 2].kind !== Kind.GR) return false
